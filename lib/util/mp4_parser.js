...
/**
 * @implements {shaka.extern.Mp4Parser}
 * @export
 */
shaka.util.Mp4Parser = class {
  ...
  static children(box) {
    ...
    // box.parser is typed as the public interface, but these callbacks are only
    // ever invoked by our own parser, whose internal state we need here.
    const parser = /** @type {!shaka.util.Mp4Parser} */ (box.parser);
    while (box.reader.hasMoreData() && !parser.done_) {
      parser.parseNext(box.start + headerSize, box.reader, box.partialOkay,
          box.stopOnPartial);
    }
  }
