...
shaka.offline.Storage = class {
   ...
   * @private
   */
  static putAllSegmentIdsIntoSet_(manifest, ids) {
  ...
  static getAllSegmentIds_(manifest) {
    /** @type {!Set.<number>} */
    const ids = new Set();
    shaka.offline.Storage.putAllSegmentIdsIntoSet_(manifest, ids);
    return Array.from(ids);
  }

  /**
   * Clean up orphaned segments and remove them from the database.
   ...
   * @export
   */
  static async removeOrphans() {
    /** @type {!shaka.offline.StorageMuxer} */
    const muxer = new shaka.offline.StorageMuxer();
    try {
      const activeHandle = await muxer.getActive();
      /** @type {!Set.<number>} */
      const segmentKeys = new Set(await activeHandle.cell.getAllSegmentKeys());

      /** @type {!Set.<number>} */
      const ownedSegmentKeys = new Set();

      const manifests = await activeHandle.cell.getAllManifests();
      manifests.forEach((manifest, key) => {
        shaka.offline.Storage.putAllSegmentIdsIntoSet_(
            manifest, ownedSegmentKeys);
      });

      /** @type {!Set.<number>} */
      const orphanedSegmentKeys = new Set();
      for (const key of segmentKeys) {
        if (!ownedSegmentKeys.has(key)) {
          orphanedSegmentKeys.add(key);
        }
      }

      await activeHandle.cell.removeSegments(
          Array.from(orphanedSegmentKeys), (key) => {});
    } finally {
      // Destroy the muxer, whether or not the operation failed.
      await muxer.destroy();
    }
  }
