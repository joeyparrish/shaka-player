...
shaka.media.MediaSourceEngine = class {
  ...
  async endOfStream(reason) {
    // EXPERIMENT, NOT FOR UPSTREAM.  Measuring whether ending the stream while
    // a seek is outstanding is what strands the seek on Safari.
    await this.waitForPendingSeek_();
  ...
  /**
   * EXPERIMENT, NOT FOR UPSTREAM.  Wait for any outstanding seek to settle.
   *
   * On Safari, a rendition switch can leave the element seeking forever.  The
   * trace shows endOfStream() landing about a millisecond after a gap jump is
   * issued on top of an already pending seek, and the seek that follows never
   * completing.  This holds endOfStream back until the element is done seeking,
   * to find out whether that overlap is what strands it.
   *
   * The wait is bounded because a seek stranded by anything else must not take
   * the player down with it.
   *
   * @return {!Promise}
   * @private
   */
  async waitForPendingSeek_() {
    if (!this.video_ || !this.video_.seeking || !this.eventManager_) {
      return;
    }
    const video = this.video_;
    const eventManager = this.eventManager_;
    await new Promise((resolve) => {
      ...
      const finish = () => {
        ...
        eventManager.unlisten(video, 'seeked', finish);
        resolve();
      };
      eventManager.listen(video, 'seeked', finish);
      new shaka.util.Timer(finish).tickAfter(
          shaka.media.MediaSourceEngine.SEEK_SETTLE_TIMEOUT_);
    });
  }
...
/**
 * EXPERIMENT, NOT FOR UPSTREAM.  Longest we will hold endOfStream() back
 * waiting for a seek to settle, in seconds.
 *
 * @const {number}
 * @private
 */
shaka.media.MediaSourceEngine.SEEK_SETTLE_TIMEOUT_ = 2;
