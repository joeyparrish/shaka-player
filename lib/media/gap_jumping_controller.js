...
shaka.media.GapJumpingController = class {
  ...
  constructor(video, timeline, config, stallDetector, onEvent) {
    ...
    this.eventManager_.listen(video, 'play', () => this.onPollGapJump_());
    this.eventManager_.listen(video, 'seeked', () => this.onPollGapJump_());

    /*
    this.eventManager_.listen(video, 'waiting', () => {
      shaka.log.info('ONWAITING', video.currentTime);
      Promise.resolve().then(() => {
        shaka.log.info('ONWAITING + PROMISE', video.currentTime);
      });
      setTimeout(() => {
        shaka.log.info('ONWAITING + 1MS', video.currentTime);
      }, 1);
      setTimeout(() => {
        shaka.log.info('ONWAITING + 100MS', video.currentTime);
      }, 100);
    });
    */
  ...
  onPollGapJump_() {
    ...
    // The current time is unbuffered (ahead of all buffered ranges)
    // or is too far from a gap (in a buffered range).
    if (gapIndex == null) {
      return;
    }
    ...
    if (isGapSmall || jumpLargeGap) {
      if (gapIndex == 0) {
        shaka.log.info(
            'Jumping from', currentTime, 'forward', jumpSize,
      ...
      this.video_.currentTime = jumpTo + 0.001;  // FIXME: rounding up
      /*
      setTimeout(() => {
        console.log('JUMP+10MS', this.video_.currentTime);
      }, 10);
      setTimeout(() => {
        console.log('JUMP+50MS', this.video_.currentTime);
      }, 50);
      setTimeout(() => {
        console.log('JUMP+100MS', this.video_.currentTime);
      }, 100);
      setTimeout(() => {
        console.log('JUMP+250MS', this.video_.currentTime);
      }, 250);
      */
    }
  }
