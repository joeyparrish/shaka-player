...
4.  **Implement Modern Bundler**: Introduce Rollup/ESBuild. The bundler simply takes each static entry point file and bundles it into its respective artifact (`shaka-player.compiled.js`, `shaka-player.ui.js`, etc.), relying on natural ES module tree-shaking rather than custom build scripts to handle conditional inclusion. Users who want custom builds can simply create their own entry point file importing just the plugins they need.
5.  **Preserve the Global Namespace**: Configure the bundler to preserve the `window.shaka` global IIFE wrapper for these generated artifacts.
