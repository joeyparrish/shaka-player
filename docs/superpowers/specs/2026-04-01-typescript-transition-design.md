...
4.  **Implement Modern Bundler & Build Flags**: Introduce Rollup/ESBuild. The bundler simply takes each static entry point file and bundles it into its respective artifact (`shaka-player.compiled.js`, `shaka-player.ui.js`, etc.), relying on natural ES module tree-shaking. The bundler must be configured with a replacement plugin (e.g., `@rollup/plugin-replace` or ESBuild's `define`) to handle compile-time constants like `shaka.Player.version`, injecting the version string from `git` or `package.json`. Because `shaka.Player` is the primary export, properties attached directly to it will not be tree-shaken.
5.  **Preserve the Global Namespace & Sunset Configurations**: Configure the bundler to preserve the `window.shaka` global IIFE wrapper for generated artifacts. Post-transition, the primary consumption method will be direct TS/ES imports, so we will sunset the vast majority of build configurations. We will retain only 1-2 core build artifacts (e.g., `compiled` and `ui`) exclusively for legacy `<script>` tag users.
